...
if (isset($_POST['add_availability'])) {
    ...
    // Optional: Check for overlapping times here

    $sql = "INSERT INTO doctor_availability (doctor_id, day_of_week, start_time, end_time) 
            VALUES ('$doctor_id', '$day', '$start', '$end')";
    mysqli_query($conn, $sql);
    header("Location: doctor_dashboard.php"); // redirect back to dashboard after adding
    exit();
}
...
if (isset($_GET['delete_avail'])) {
    ...
    mysqli_query($conn, "DELETE FROM doctor_availability WHERE id='$id' AND doctor_id='$doctor_id'");
...
if (isset($_GET['delete_unavail'])) {
    ...
    mysqli_query($conn, "DELETE FROM doctor_unavailability WHERE id='$id' AND doctor_id='$doctor_id'");
    header("Location: doctor_dashboard.php");
    exit();
}
...
<head>
    <meta charset="utf-8">
    <title>My Availability - <?php echo htmlspecialchars($doctor['name']); ?></title>
...
<div class="container mt-4">
    <h3 class="text-primary mb-4">
        <i class="bi bi-calendar-check"></i> Manage My Availability
    </h3>
    <a href="doctor_dashboard.php" class="btn btn-secondary btn-sm mb-3">
    ...
    <table class="table table-striped table-hover">
        ...
        <?php while ($row = mysqli_fetch_assoc($availabilities)) { ?>
            <tr>
                ...
                <td>
                    <a href="?delete_avail=<?php echo $row['id']; ?>" 
    ...
    <table class="table table-striped table-hover">
        ...
        <?php while ($row = mysqli_fetch_assoc($unavailabilities)) { ?>
            <tr>
                ...
                <td>
                    <a href="?delete_unavail=<?php echo $row['id']; ?>" 
